...
int floor_color(t_data *data, int row, double dist_proj_plane, int height_proj_plane)
{
	...
	{
		ratio.x = 64 - (int)sol.x % BLOCK_SIZE;
		ratio.y = 64 - (int)sol.y % 64;
	}
...
int fill_column(t_data *data)
{
	...
	{
		*(int*)(data->image.add_image + (row * data->image.size_line) + (data->raycast.column * sizeof(int))) = floor_color(data, row, dist_proj_plane, wall_row);
		wall_row++;
		row++;
	}
	return (0);

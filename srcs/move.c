...
int move_x(t_data *data, int value)
{
	...
	if (tmp < (data->world.size.x * BLOCK_SIZE) &&	tmp > BLOCK_SIZE)
	{
		...
		else if (data->world.map[data->player.pos.y / BLOCK_SIZE][(tmp / BLOCK_SIZE)] == '2' && ((tmp%BLOCK_SIZE > BLOCK_SIZE/8) && tmp%BLOCK_SIZE < BLOCK_SIZE - BLOCK_SIZE/8))
			data->player.pos.x = tmp;
	}
	return (0);
}
...
int move_y(t_data *data, int value)
{
	...
	if (tmp < (data->world.size.y * BLOCK_SIZE) &&	tmp > BLOCK_SIZE)
	{
		...
		else if (data->world.map[tmp / BLOCK_SIZE][data->player.pos.x / BLOCK_SIZE] == '2' && !((tmp%BLOCK_SIZE > BLOCK_SIZE/8) && (tmp%BLOCK_SIZE < BLOCK_SIZE - BLOCK_SIZE/8)))
			data->player.pos.y = tmp;
	}
	return (0);
}
